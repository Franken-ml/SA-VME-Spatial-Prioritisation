...
targets_VMEs_catch <- tibble(
  ...
  mutate(target = if_else(feature == "Suberites_VME_catch", 0.30, 0.30))
...
solution_30 <- solve(problem_01, force = TRUE)

plot(solution_30[, "solution_1"], main = "Solution 30")


# Create visualization plots
ggplot() +
  geom_sf(data = solution_30, aes(fill = factor(solution_1))) +
  geom_sf(data = EEZ_SA_transformed, fill = "lightblue", alpha = 0.3) +
  scale_fill_manual(values = c("white", "darkgreen"),
                    labels = c("Not Selected", "Selected")) +
  labs(title = "Conservation Solution for Scenario 1 (30x30)",
       fill = "Status") +
  theme_minimal()

####################### EXPORT SOLUTION 30 RESULTS #######################
...
solution_stats <- tibble(
  ...
  value = c(
    as.numeric(sum(st_area(solution_30[solution_30$solution_1 == 1,]))) / 1000000,
    sum(solution_30$solution_1),
    sum(solution_30$cost[solution_30$solution_1 == 1])
  )
)
...
feature_stats <- data.frame(
  ...
  protected_area = sapply(names_features, function(f) 
    sum(PUs_all_features_cost[[f]][solution_30$solution_1 == 1] > 0)),
...
target_achievement <- problem_01 %>%
  eval_target_coverage_summary(data.frame(solution_1 = solution_30$solution_1))

# Save results
dir.create("Outputs/Results", recursive = TRUE, showWarnings = FALSE)
write.csv(solution_stats, "Outputs/Results/solution30_statistics.csv")
write.csv(feature_stats, "Outputs/Results/solution30_feature_representation.csv")
write.csv(target_achievement, "Outputs/Results/solution30_target_achievement.csv")
